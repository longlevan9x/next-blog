...
const EmbedBlock = ({url}: EmbedBlockProps) => {
    ...
    if (link.hostname.includes("facebook.com")) {
        ...
        if (isDomainsOfEmbed) {
            ...
            } else if (link.hostname.includes("forms.gle") || (link.hostname.includes("docs.google.com") && link.pathname.includes("/forms/"))) {
                classNames = "max-w-[650px] max-h-[800px] md:h-[600px]"
            }
            const afterUrl = link.toString();
            cpn = <Iframe title={`iFrame ${link.hostname}`} className={`w-full h-[500px] ${classNames}`} url={afterUrl}
